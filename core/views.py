...
def add_game(request):
    if request.method == 'POST':
        ...
        if not vip_price:
            errors['vip_price'] = "VIP narxni kiriting"
...
def start_game(request, pk):
    ...
    if request.method == 'POST':
        if 'mark_as_paid' in request.POST:
            ...
            return redirect('start_game', pk=game.pk)  # Sahifa qayta yuklanadi

        tariff = request.POST.get('tariff', 'standard')

        # Yangi o‘yin sessiyasi yaratish
        session = GameSession(
            game=game,
            tariff=tariff,
            created_by=request.user
        ...
        messages.success(request, f"{game.name} o‘yini {dict(GameSession.TARIFF_CHOICES)[tariff]} tarifi bilan boshlandi!")
